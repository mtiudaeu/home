...
static void shader_gl_log_error(GLuint object) {
  ...
  LOG_ERROR("shader_gl_log_error", std::string(log));
}
...
static char* shader_file_read_malloc(const char* filename) {
  ...
  if (!f) {
    LOG_ERROR("fopen", strerror(errno), filename);
    return 0x0;
  }

  size_t err = fseek(f, 0, SEEK_END);
  if (err) {
    LOG_ERROR("fseek", strerror(errno), filename);
    return 0x0;
  }
  ...
  err = fclose(f);
  if (err) {
    LOG_ERROR("fclose", strerror(errno), filename);
  }
...
GLuint shader_texture_buffer_create(const char* image_file_path) {
  ...
  if (!tileset_texture) {
    LOG_ERROR("IMG_Load: ", SDL_GetError(), image_file_path);
    return tbo_texture;
  }
