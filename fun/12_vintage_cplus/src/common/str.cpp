...
struct str_buf str_buf_create(size_t size, struct allocator_cbs allocator_cbs)
{
 ...
 char* data = (char*)allocator_cbs.malloc(size*size_of_one);

 struct str_buf str_buf = dyn_buf_create(char, size, allocator_cbs);
/*
...
void str_buf_append(struct str_buf* str_buf_ptr, struct str str)
{
 ...
 if(new_size > str_buf_ptr->dyn_buf_info.capacity)
 {
  ...
  str_buf_ptr->data = (char*)str_buf_ptr->dyn_buf_info.allocator_cbs.realloc(str_buf_ptr->data, new_capacity * str_buf_ptr->dyn_buf_info.size_of_one);
