...
//--------------------------------------------------------------------------------
static float _graphics_text_square_vertices_width(float scale) {
  const float size_ratio = 0.1f;
  return scale * size_ratio;
}

//--------------------------------------------------------------------------------
static void _graphics_text_rectangle_2D(struct rectangle_2d* rectangle_2D, float scale,
                             struct math_vec2 position) {
  assert(rectangle_2D);

  rectangle_2D->x = position.x;
  rectangle_2D->y = position.y;
  rectangle_2D->width = _graphics_text_square_vertices_width(scale);
  // FIXME add height.
  rectangle_2D->height = _graphics_text_square_vertices_width(scale);
}
...
//--------------------------------------------------------------------------------
static struct grid_16x16 _graphics_text_char_to_grid_coord(char value) {
...
static void _graphics_text_rectangle_2D_texture(struct rectangle_2d* rectangle_2D, char value) {
  assert(rectangle_2D);

  const struct grid_16x16 character_coord = _graphics_text_char_to_grid_coord(value);
