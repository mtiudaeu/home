...
def build_object_rules(c_file_path_list):
  ...
  for c_file_path in c_file_path_list:
    ...
    objects_files.append(o_file_target)
    makefile_path.write(o_file_target + ": " + c_file_path + " " + c_file_target + "\n")
    makefile_path.write("\t$(CXX) $(" + var_all_obj_flags + ") -c $< -o $@\n")

  objects_files = " ".join(objects_files)
  makefile_path.write(var_all_obj_file + "=" + objects_files + "\n")
...
var_all_obj_file = sys.argv[5]
var_all_obj_flags = sys.argv[6]
